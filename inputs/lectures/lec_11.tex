%! TEX root = ../../master.tex
\lecture[Metrisierbarkeit des Hilbertraums. Beweis des Metrisierungssatzes von Urysohn. Motivation für die Einführung von Algebraischer Topologie sowie Kategiorientheorie. Mengen und Klassen. Anfänge der Kategorientheorie: Kategorien, Unterkategorien, Isomorphismen, Funktoren.]{Do 20 Mai 2021}{Kategorientheorie}
...
\begin{proof}
    ...
    \[
        D((x_n)_{n\in \mathbb{N}}, (y_n)_{n\in \mathbb{N}}) = \sup \left \{\frac{\abs{x_n-y_n}}{n}\mid n\in \mathbb{N} \right\} 
    .\] 
\end{proof}
...
\begin{lemma}\label{lm:abzählbare-trennungsfamilie-in-normalem-raum-mit-abzählbarer-basis}
   ...
   .\] 
   sodass für jedes $x\in X$ und jede offene Umgebung $x\in U$ ein $i\in \mathbb{N}$ existiert, sodass $f_i(x) = 1$ und  $f_i(y) = 0$ für  $y\not\in U$.
\end{lemma}
...
\begin{doral}
    Für den Beweis von \autoref{thm:metrisierungsssatz-von-urysohn} brauchen wir nicht wirklich, dass wir eine abzählbare Basis finden, sondern es genügt die Eigenschaft ebigen Lemmas. Die abzählbare Basis ist jedoch die einfachste Eigenschaft das zu garantieren.
\end{doral}

\begin{proof}[Beweis des \nameref{thm:metrisierungsssatz-von-urysohn}]
    Seien $(f_i \colon X \to  [0,1])_{i\in \mathbb{N}}$ wie in \autoref{lm:abzählbare-trennungsfamilie-in-normalem-raum-mit-abzählbarer-basis}. Definiere
        \begin{equation*}
        F: \left| \begin{array}{c c l} 
            X & \longrightarrow & \prod_{i=1}^{\infty}[0,1] \\
            x & \longmapsto &  (f_i(x))_{i\in \mathbb{N}}
        \end{array} \right.
    \end{equation*}
    ...
    \begin{subproof}
        ...
        \begin{itemize}
            \item Seien $x\neq y\in X$. Da $X$ normal ist, finden wir eine offene Menge $x\in U$, $y\not\in U$ (erneut, indem wir uns erinnern, dass normale Räume Hausdorff sind, und dann \autoref{thm:hausdorff-impliziert-t1} anwenden). Wegen \autoref{lm:abzählbare-trennungsfamilie-in-normalem-raum-mit-abzählbarer-basis} gibt es also $n\in \mathbb{N}$ mit $f_n(x) \subset f_n(U) = 1$ und  $f_n(X\setminus U) = 0$, also 
                \[
                    f_n(x) = 1 \neq  f_n(y) \implies F(x) \neq  F(y)
                .\] 
                Also ist $F$ injektiv.
            \item Sei $U\subset X$ offen. Wir zeigen: $F(U)\subset \prod_{\mathbb{N}}$ ist offen. Sei $z\in F(U)$ mit (eindeutigem) Urbild $x\in U$. Wir konstruieren eine Menge $V\subset \prod_{\mathbb{N}}[0,1]$ offen, sodass $z\in V \cap F(X)\subset F(U)$, dann ist $F(U)$ offen in  $F(X)$. \\
                ...
                .\] 
                als offene Teilmenge von $\prod_{\mathbb{N}}[0,1]$, wobei $(0,1]$ im  $n$-ten Faktor stehe.
                \begin{claim}
...
\begin{example}
    \begin{itemize}
        ...
        \item $\pi_0(\mathbb{N}) = \left \{\left \{n\right\} \mid n\in \mathbb{N}\right\}$, weil die einzigen wegzusammenhängenden Teilmengen von $\mathbb{N}$ die einpunktigen Mengen sind
...
Wir fordern neben den üblichen Axiomen von \textbf{ZFC} (hierbei steht \textbf{C} für das sogenannte \vocab{Auswahlaxiom}), noch die Existenz mehrerer unerreichbarer Kardinalzahlen (d.h. welche außer $\aleph_0 \coloneqq  \card(\mathbb{N})$, die wir $\aleph_0<κ<κ'$ nennen)
...
\begin{definition}[Kategorie]\label{def:kategorie}
    Eine \vocab{Kategorie} $\mathcat{C}$ besteht aus
    \begin{itemize}
        \item Einer Klasse von \vocab[Kategorie!Objekt]{Objekten}, notiert $\Ob(\mathcat{C})$.
        \item $\forall X,Y \in \Ob(\mathcat{C})$ eine Menge $\Mor_{\mathcat{C}}(X,Y)$ von \vocab[Kategorie!Morphismus]{Morphismen}
...
\begin{remark*}[Assoziativität und kommutative Diagramme]
    Mit Assoziativität meinen wir das folgende: Sind $X,Y,Z,W\in \Ob(\mathcat{C})$ und $f\in \Mor_{\mathcat{C}}(X,Y)$, $g\in \Mor_{\mathcat{C}}(Y,Z), h\in \Mor_{\mathcat{C}}(Z,W)$, so ist $h \circ  (g \circ f) = (h \circ  g) \circ  f$. Wir veranschaulichen dies in einem \vocab[Kommutatives Diagramm]{kommutativen Diagramm} (das ist typische für die Kategorientheorie, wir malen Objekte als Punkte und Elemente von $\Mor_{\mathcat{C}}(X,Y)$ als Pfeile von $X\to Y$, so sollte man sich das vorstellen):
    \[
...
\begin{definition}[Unterkategorie]\label{def:unterkategorie}
    \begin{itemize}
        \item $\mathcat{U}$ ist eine \vocab[Kategorie!Unter-]{Unterkategorie} von $\mathcat{C}$, falls $\mathcat{U}$ eine Kategorie ist mit $\Ob(\mathcat{U}) \subset \Ob(\mathcat{C})$, und $\forall X,Y\in \Ob(\mathcat{U})$ ist
    \[
        \Mor_{\mathcat{U}}(X,Y) \subset \Mor_{\mathcat{C}}(X,Y)
    .\] 
\item Ist zudem für $X,Y \in \Ob(\mathcat{U})$ obige Inklusion sogar eine Gleichheit, d.h. $\Mor_{\mathcat{U}}(X,Y) = \Mor_{\mathcat{C}}(X,Y)$, so heißt $\mathcat{U}$ \vocab[Kategorie!Unter-!volle]{volle} Unterkategorie. 
    \end{itemize}
\end{definition}
...
\begin{definition}[Isomorphismus]\label{def:isomorphismus}
    $f\in \Mor_{\mathcat{C}}(X,Y)$ ist ein \vocab[Kategorie!Isomorphismus]{Isomorphismus}, wenn es ein $g\in \Mor_{\mathcat{C}}(Y,X)$ gibt mit $f \circ  g = \id_Y$ und $g \circ f = \id_X$ (d.h. eine Umkehrabbildung).
\end{definition}
...
\begin{definition}[Funktor]\label{def:funktor}
    Seien $\mathcat{C},\mathcat{D}$ Kategorien. Ein (kovarianter) \vocab[Kategorie!Funktor]{Funktor} $\mathcal{F} \colon \mathcat{C} \to  \mathcat{D}$ besteht aus:
    \begin{itemize}
...
\begin{definition}[Isomorphismus von Kategorien]\label{def:funktor-isomorphismus}
    Ein Funktor $\mathcal{F} \colon \mathcat{C} \to  \mathcat{D}$ ist ein \vocab{Isomorphismus}, falls es einen Funktor $\mathcal{G} \colon \mathcat{D} \to  \mathcat{C}$ gibt, sodass $\mathcal{G} \circ  \mathcal{F}$ und $\mathcal{F} \circ  \mathcal{G}$ die Identitäten sind.
\end{definition}
\begin{oral}
...
\begin{example}
    \begin{itemize}
        ...
        \item Betrachte den Funktor
                ...
                X & \longmapsto &  \mathbb{Z}[X] \\
                f: X \to  Y &\longmapsto & 
                    \left| \begin{array}{c c l} 
                        \mathbb{Z}[X] & \longrightarrow & \mathbb{Z}[Y] \\
                        \sum\limits_{i=1}^k n_i x_i& \longmapsto	 & \sum\limits_{i=1}^k n_i f(x_i)
                \end{array} \right.
                
                \end{array} \right.
            \end{equation*}
            Wir senden hierbei eine Menge $X$ auf die \vocab{freie abelsche Gruppe} , die von $X$ generiert wird. Dabei ist $\mathbb{Z}[X]$ das (freie) $\mathbb{Z}$-Modul, das als Basis  von $X$ erzeugt wird.
    \end{itemize}
\end{example}
