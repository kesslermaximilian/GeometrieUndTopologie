%! TEX root = ../../master.tex
\lecture[Torus, Kleinsche Flasche, Reeller Projektiver Raum. Trennungsaxiome: Hausdorff, normale und reguläre Räume. Kompaktheit. ]{Di 20 Apr 2021}{Trennungsaxiome, Kompaktheit}
\begin{proof}
...
\end{remark}
\begin{example}\label{ex:quotientenräume}
    \begin{enumerate}[1)]
...
\captionof{figure}{Konstruktion des reellen projektiven Raums für den Fall $n=1$. Wir identifizieren die roten Strahlen miteinander, nicht jedoch den gesamten blauen, da $λ>0$.}
        ...
        \item Sei $X$ ein topologischer Raum und  $A\subset X$ eine Teilmenge. Definiere die Relation $\sim $ durch $a\sim a'$ für $a,a'\in A$ (bzw. erzeuge eine dadurch). Dann setzen wir
            ...
            .\] 
            Damit ergibt sich
        \item\label{item:sierpinski-raum-als-quotient-von-einheitsintervall} $[0,1] / \left[0,1\right) \cong S$  ist der Sierpinski-Raum, vergleiche auch \autoref{ex:sierpinski-raum}.
    \begin{proof}
...
\begin{remark}
    Quotientenräume von metrischen Räumen sind im Allgemeinen nicht metrisierbar, \autoref{ex:quotientenräume} \autoref{item:sierpinski-raum-als-quotient-von-einheitsintervall} liefert hier ein gewünschtes Gegenbeispiel.
\end{remark}
...
\begin{remark*}
    ...
    Im Gegensatz zum Hausdorff-Raum trennen wir zwei Punkte also durch 2 nicht notwendigerweies disjunkte Umgebungen / offene Mengen. Mit dem gleichen Beweis wie in \autoref{thm:hausdorff-impliziert-t1} zeigen wir dann, dass jeder Punkt abgeschlossen ist. Ist umgekehrt $X$ ein Raum, in dem alle Punkte abgeschlossen sind, so können wir  $x,y$ stets durch die offenen Umgebungen  $y\in X \setminus \left \{x\right\} $ sowie $x\in X \setminus \left \{y\right\} $ trennen. Die beiden Definitionen sind also äquivalent.
...
\begin{proof}
    ...
    \[
    U_x \cap A \qquad \text{und} \qquad U_y \cap A \subset A
    \] 
