...
\begin{example}['Gegenbeispiel' zum \nameref{thm:allgemeiner-liftungssatz}]
    ...
    \begin{minipage}{\textwidth}
        \centering
        \includegraphics[scale=0.25]{figures/handdrawn/gegenbeispiel-allgemeiner-liftungssatz.png}
    \end{minipage}
...
\begin{example}[Zusammenhängende Überlagerungen der $S^1$]\label{ex:zusammenhängende-überlagerungen-der-s1}
    ...
    Wir erhalten also, dass zwei solche Überlagerungen $E\to S^1$ genau dann homöomorph (über $S^1$) sind, wenn die zugehörigen charakteristischen Untergruppen (an Punkten aus der Faser von $1\in S^1$) identisch sind. Diese sind aber stets Untergruppen von $\pi_1(S^1,1) = \mathbb{Z}$, also klassifizieren wir diese:

    Die Untergruppen von $\mathbb{Z}$ sind genau $\left \{0\right\} $ (die triviale Gruppe) sowie die Gruppen $n\mathbb{Z}$ für $n\geq 1$. Wir erhalten in der Tat
    \begin{itemize}
        ...
        \item Für $n\mathbb{Z}$ die Überlagerung
                \begin{equation*}
        ...
        Diese induziert den Gruppenhomomorphismus
            \begin{equation*}
            \begin{array}{c c l} 
                \pi_1(S^1,1) = \mathbb{Z} & \longrightarrow & \pi_1(S^1,1) = \mathbb{Z} \\
            1 & \longmapsto &  n
            \end{array}
        \end{equation*}
        und hat somit charakteristische Untergruppe $n\mathbb{Z}\subset \mathbb{Z}$.
    ...
    Als Beispiel fällt einem noch die Überlagerung
        ...
        \end{array} \right.
    \end{equation*}
    ein, die  $\mathbb{Z} \to  \mathbb{Z}$ mit $1 \mapsto -n$ induziert, also keinen der vorherigen Morphismen. Die charakteristische Fundamentalgruppe ist aber ebenfalls  $n\mathbb{Z}$, also muss diese Überlagerung nach \autoref{thm:äquivalenz-von-überlagerungen-über-lokal-wegzsuammenhängendem-wegzusammenhängendem-raum} homöomorph zu $()^k$ sein. In der Tat ist auch
        \begin{equation*}
        \begin{array}{c c l} 
        S^1 & \longrightarrow & S^1 \\
